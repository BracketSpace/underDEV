...
class Requirements {
	...
	public function check_php_extensions( $extensions, $requirements ) {
		...
		if ( ! empty( $missing_extensions ) ) {
			$requirements->add_error( sprintf(
				_n( 'PHP extension: %s', 'PHP extensions: %s', count( $missing_extensions ), 'notification-slugnamexx' ),
	...
	public function check_function_collision( $functions, $requirements ) {
		...
		if ( ! empty( $collisions ) ) {
			$requirements->add_error( sprintf(
				_n( 'register %s function but it\'s already taken', 'register %s functions but these are already taken', count( $collisions ), 'notification-slugnamexx' ),
				implode( ', ', $collisions )
			) );
		}
	...
	public function check_class_collision( $classes, $requirements ) {
		...
		if ( ! empty( $collisions ) ) {
			$requirements->add_error( sprintf(
				_n( 'register %s class but it\'s already defined', 'register %s classes but these are already defined', count( $collisions ), 'notification-slugnamexx' ),
				implode( ', ', $collisions )
			) );
		}
